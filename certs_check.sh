...
str=`openssl x509 -enddate -noout -in ./IT-Planet_Network_Dychakovskaya-2/pki/ca.crt`
...
if [ ${diff[0]} -eq 0 ] && [ ${diff[1]} -le 1 ] && [ ${diff[2]} -eq 0 ] ; 
    then
        ...
        rm -rf ./pki/ca.crt
        docker compose -f ./IT-Planet_Network_Dychakovskaya-2/docker-compose.yaml up certs_openssl # УКАЖИ ПУТЬ
        ./installing_certs.sh
